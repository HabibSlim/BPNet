...
class FullLoader(CompatLoader2D):
    ...
    def __init__(
        ...
        super().__init__(
            root_url, split, sem_level, n_comp, cache_dir, view_type, transform
        )
...
class LinkCreator(object):
    ...
    def computeLinking(self, world_to_camera, coords, depth):
        ...
        inside_mask = (
            (pi[0] >= 0)
            * (pi[1] >= 0)
            * (pi[0] < self.imageDim[0])
            * (pi[1] < self.imageDim[1])
        )
        occlusion_mask = (
            np.abs(depth[pi[1][inside_mask], pi[0][inside_mask]] + p[2][inside_mask])
            <= self.voxel_size
        )
...
def collation_fn(batch, V=8, eval_all=False):
    ...
    colors = colors.view(
        -1, V, colors.shape[-3], colors.shape[-2], colors.shape[-1]
    ).permute(0, 2, 3, 4, 1)
    part_2d = part_2d.view(-1, V, part_2d.shape[-2], part_2d.shape[-1]).permute(
        0, 2, 3, 1
    )
    mat_2d = mat_2d.view(-1, V, mat_2d.shape[-2], mat_2d.shape[-1]).permute(0, 2, 3, 1)
    obj_cls = (
        torch.from_numpy(obj_cls).long().view(-1, V)[:, 0]
    )  # only need one label for all views
    ...
    return (
        torch.cat(coords_all).int(),
        torch.cat(feat_all).float(),
        torch.cat(part_3d_all).long(),
        colors.float(),
        part_2d.long(),
        torch.cat(links_all),
        obj_cls,
        mat_2d.long(),
        torch.cat(mat_3d_all).long(),
        model_id,
        inds_recons_all,
        style_id,
        view_id,
...
class GCRLoader3D(CompatLoader2D):
    ...
    def load_point_cloud(self, model_id, style_id):
        idx = np.where(
            (self.all_shape_ids == model_id) & (self.all_style_ids == style_id)
        )[0][0]
    ...
    def process_item(self, item, voxelSize=0.05, aug=True):
        ...
        transform_2d = get_transform_2d(
            aug=aug, IMG_DIM=IMG_DIM, value_scale=1.0
        )  # scaled to 1 when decode in webdataset
    ...
    def collation_view(self, batch, V=8, voxelSize=0.05, aug=True, eval_all=False):
        ...
        MAX_NUM_POINTS = (
            locs_in.shape[0] * 1
        )  # maximum points filtered from different view points

        links = links.permute(1, 2, 0)

        #### preparing 3D data
        voxelizer, prevoxel_transforms, input_transforms = get_transforms_3d(
            voxelSize=voxelSize
        )
        locs = prevoxel_transforms(locs_in) if aug else locs_in
        locs, feats, part_mat_3d, inds_recons, links = voxelizer.voxelize(
            locs, feats_in, labels_in, link=links
        )
        ...
        if not eval_all:
            part_mat_3d = F.pad(
                part_mat_3d, (0, 0, 0, MAX_NUM_POINTS - N), mode="constant", value=-1
            )
        links = F.pad(
            links, (0, 0, 0, 0, 0, MAX_NUM_POINTS - N), mode="constant", value=-1
        )
    ...
    def make_loader(
        ...
        if distributed:
            ...
            number_of_batches = int(
                np.ceil(dataset_size / (batch_size * world_size * self.view_num))
            )  ### Note that batch_size is the number of shapes
            print(
                "{} dataset_size, batch_size, world_size, self.view_num: ".format(
                    self.split
                ),
                dataset_size,
                batch_size,
            ...
            #             if self.split=='train':
            loader = loader.repeat(2).slice(
                number_of_batches
            )  # If number_of_batches is interger, then this do nothing, it works like keep_last in torch dataloader
