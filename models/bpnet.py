...
class BPNet(nn.Module):
    def __init__(self, cfg=None):
        ...
        # 3D
        net3d = constructor3d(
            in_channels=3, out_channels=cfg.classes, D=3, arch=cfg.arch_3d
        )
        self.layer0_3d = nn.Sequential(net3d.conv0p1s1, net3d.bn0, net3d.relu)
        self.layer1_3d = nn.Sequential(
            net3d.conv1p1s2, net3d.bn1, net3d.relu, net3d.block1
        )
        self.layer2_3d = nn.Sequential(
            net3d.conv2p2s2, net3d.bn2, net3d.relu, net3d.block2
        )
        self.layer3_3d = nn.Sequential(
            net3d.conv3p4s2, net3d.bn3, net3d.relu, net3d.block3
        )
        self.layer4_3d = nn.Sequential(
            net3d.conv4p8s2, net3d.bn4, net3d.relu, net3d.block4
        )
        self.layer5_3d = nn.Sequential(net3d.convtr4p16s2, net3d.bntr4, net3d.relu)
        self.layer6_3d = nn.Sequential(
            net3d.block5, net3d.convtr5p8s2, net3d.bntr5, net3d.relu
        )
        self.layer7_3d = nn.Sequential(
            net3d.block6, net3d.convtr6p4s2, net3d.bntr6, net3d.relu
        )
        self.layer8_3d = nn.Sequential(
            net3d.block7, net3d.convtr7p2s2, net3d.bntr7, net3d.relu
        )
        ...
        assert not (
            self.use_2d_classifier & self.use_3d_classifier
        )  # in general, we only choose one shape classifier
    ...
    def forward(self, sparse_3d, images, links):
        ...
        if self.use_2d_classifier:
            ...
            if self.pool2d:
                x6 = torch.max(
                    F.max_pool2d(x5, (h // 32, w // 32)).squeeze().view(v, b, -1), 0
                )[0].view(b, -1)
            else:
                x6 = torch.max(
                    x5.view(v, b, x5.shape[-3], x5.shape[-2], x5.shape[-1]), 0
                )[0].view(b, -1)
        ...
        links_current_level[:, 1:3, :] = (
            (H - 1.0) / (h - 1.0) * links_current_level[:, 1:3, :].float()
        ).int()
        fused_3d_p5, fused_2d_p5 = self.linker_p5(
            x5, out_b4p16, links_current_level, init_3d_data=sparse_3d
        )

        p4 = self.up4_2d(
            F.interpolate(
                fused_2d_p5, x4.shape[-2:], mode="bilinear", align_corners=True
            )
        )
        ...
        fused_3d_p4, fused_2d_p4 = self.linker_p4(
            p4, feat_3d, links_current_level, init_3d_data=sparse_3d
        )

        p3 = self.up3_2d(
            F.interpolate(
                fused_2d_p4, x3.shape[-2:], mode="bilinear", align_corners=True
            )
        )
        p3 = torch.cat([p3, x3], dim=1)
        p3 = self.delayer3_2d(p3)
        feat_3d = self.layer6_3d(
            ME.cat(fused_3d_p4, out_b3p8)
        )  # corresponding to FPN p3
        ...
        fused_3d_p3, fused_2d_p3 = self.linker_p3(
            p3, feat_3d, links_current_level, init_3d_data=sparse_3d
        )

        p2 = self.up2_2d(
            F.interpolate(
                fused_2d_p3, x2.shape[-2:], mode="bilinear", align_corners=True
            )
        )
        p2 = torch.cat([p2, x2], dim=1)
        p2 = self.delayer2_2d(p2)
        feat_3d = self.layer7_3d(
            ME.cat(fused_3d_p3, out_b2p4)
        )  # corresponding to FPN p2
        ...
        fused_3d_p2, fused_2d_p2 = self.linker_p2(
            p2, feat_3d, links_current_level, init_3d_data=sparse_3d
        )
        ...
        res_mat = F.interpolate(
            res_mat, size=(h, w), mode="bilinear", align_corners=True
        )
