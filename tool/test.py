...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.data_name == "3dcompat":
        ...
        val_loader = GCRLoader3D(
            root_url=ROOT_URL,
            split="valid",
            n_comp=args.com,
            view_type=args.view_type,
            sem_level=args.sem_level,
        ).make_loader(
...
def validate_cross(model, val_loader):
    torch.backends.cudnn.enabled = (
        False  # for cudnn bug at https://github.com/pytorch/pytorch/issues/4107
    )
    ...
    target_meter_3dmat, union_meter_3dmat, intersection_meter_3dmat = (
        AverageMeter(),
        AverageMeter(),
        AverageMeter(),
    )
    target_meter_mat, union_meter_mat, intersection_meter_mat = (
        AverageMeter(),
        AverageMeter(),
        AverageMeter(),
    )
    ...
    with torch.no_grad():
        for i, batch_data in tqdm(enumerate(val_loader)):
            ...
            intersection, union, target = intersectionAndUnionGPU(
                output_3d, label_3d.detach(), args.classes, args.ignore_label
            )
            if args.multiprocessing_distributed:
            ...
            intersection, union, target = intersectionAndUnionGPU(
                output_2d, label_2d.detach(), args.classes, args.ignore_label
            )
            if args.multiprocessing_distributed:
                dist.all_reduce(intersection), dist.all_reduce(union), dist.all_reduce(
                    target
            ...
            accuracy_2d = sum(intersection_meter_2d.val) / (
                sum(target_meter_2d.val) + 1e-10
            )
            ...
            intersection, union, target = intersectionAndUnionGPU(
                output_3dmat, mat3d.detach(), args.mat, args.ignore_label
            )
            intersection, union, target = (
                intersection.cpu().numpy(),
                union.cpu().numpy(),
            ...
            intersection, union, target = intersectionAndUnionGPU(
                output_mat, mat.detach(), args.mat, args.ignore_label
            )
            if args.multiprocessing_distributed:
                dist.all_reduce(intersection), dist.all_reduce(union), dist.all_reduce(
                    target
    ...
    accuracy_class_3dmat = intersection_meter_3dmat.sum / (
        target_meter_3dmat.sum + 1e-10
    )
    mIoU_3dmat = np.mean(iou_class_3dmat)
    mAcc_3dmat = np.mean(accuracy_class_3dmat)
    allAcc_3dmat = sum(intersection_meter_3dmat.sum) / (
        sum(target_meter_3dmat.sum) + 1e-10
    )
    ...
    if main_process():
        ...
        logger.info(
            "Val result 2d : mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.".format(
                mIoU_2d, mAcc_2d, allAcc_2d
            )
        )
        logger.info(
            "Val result 2dmat: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.".format(
                mIoU_mat, mAcc_mat, allAcc_mat
            )
        )
        logger.info(
            "Val result 3dmat: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.".format(
                mIoU_3dmat, mAcc_3dmat, allAcc_3dmat
            )
        )
        logger.info("Class ACC{:.4f}".format(acc_cls))

        print(
            "pred_parts.shape, gt_parts.shape, pred_mats.shape, gt_mats.shape",
            pred_parts.shape,
            gt_parts.shape,
            pred_mats.shape,
            gt_mats.shape,
